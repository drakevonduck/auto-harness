...
license: Apache-2.0
compatibility: For any Agent Skills-compatible client (Claude Code, VS Code Copilot, Cursor, WindSurf, and others). Target repository may use any stack. The harness platform must be accessible at platform/ relative to the harness repo root, or via the PLATFORM environment variable.
metadata:
